...
public class TiledTestSuperKoalio extends InputAdapter implements ApplicationListener {
    ...
    public void create() {
        ...
        koalaTexture = new Texture("maps/super-koalio/koalio.png");
        ...
        map = new TmxMapLoader().load("maps/super-koalio/level1.tmx");
    ...
    private void updateKoala (float deltaTime) {
        ...
        for (Rectangle tile : tiles) {
            if (koalaRect.overlaps(tile)) {
                ...
                // so it is just below/above the tile we collide with
                // this removes bouncing
                if (koala.velocity.y > 0) {
                    koala.position.y = tile.y - Koala.HEIGHT;
                    // we hit a block jumping upwards, let's destory it
        ...
        // apply damping to the velocity on the x-axis so we don't walk infinitely
    ...
    private void renderKoala (float deltaTime) {
        // base on koala state, get the animation frame
        ...
        // draw the koala, depending on current velocity
    ...
    private void renderDebug() {
        ...
        for (int y = 0; y <= layer.getHeight(); y++) {
            for(int x = 0; x <= layer.getWidth(); x++) {
                TiledMapTileLayer.Cell cell = layer.getCell(x, y);
                if (cell != null) {
                    if(camera.frustum.boundsInFrustum(x  + 0.6f, y+0.5f, 0, 1, 1, 0))
